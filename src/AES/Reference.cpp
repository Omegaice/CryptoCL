...
namespace AES {
	/* Public Functions */
	const CharArray Reference::Encrypt( const CharArray& data ) {
		CharArray result, lData( data.begin(), data.end() );
	...
	const CharArray Reference::Decrypt( const CharArray& data ) {
		CharArray result, lData( data.begin(), data.end() );
	...
	void Reference::AddRoundKey( const unsigned int round ) {
		const unsigned int sRount = round * 16;
		const unsigned int eRound = round * 16 + 16;
		const CharArray RoundKey( mRoundKey.begin() + sRount, mRoundKey.begin() + eRound );
	...
	void Reference::ShiftRows() {
		const CharArray TempState( mState.begin(), mState.end() );
	...
	CharArray MixColumn( const CharArray& column ) {
		CharArray result( 4 );
	...
	void Reference::MixColumns() {
		for( unsigned int col = 0; col < 4; col++ ){
			...
			const CharArray result = MixColumn( column );
	...
	CharArray InvMixColumn( const CharArray& column ){
		CharArray result( 4 );
	...
	void Reference::InvMixColumns() {
		for( unsigned int col = 0; col < 4; col++ ){
			...
			const CharArray result = InvMixColumn( column );
