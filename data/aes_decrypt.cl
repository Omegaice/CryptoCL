...
__kernel void decryptUnoptimized( __global const uchar *rkey, __global const uchar *data, __global uchar *result, const uint rounds ) {
...
__kernel void decrypt( __global const uchar *rkey, __global const uchar *data, __global uchar *result, const uint rounds ) {
	...
	for( uint i = 0; i < 16; i++ ) result[startPos+i] = block[i] ^ rkey[i];
}
